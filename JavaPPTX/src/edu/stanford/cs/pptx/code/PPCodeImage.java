 ...
 *
 * @return An array of lines containing the code
 */
 ...
 */
   ...
   public Rectangle2D getCodeBounds(int line, int p1, int p2) {
      ...
      FontMetrics fm = getScaledMetrics(font);
      double x = getX() + leftMargin + fm.stringWidth(prefix) / sf + X_DELTA;
      double y = getY() + topMargin + line * spacing + Y_DELTA;
      str = str.substring(p1, p2);
      double width = fm.stringWidth(str) / sf + W_DELTA;
...
/**
 * Paints the code image using the graphics context.
 *
 ...
 */
   ...
   protected void paintCodeImage(Graphics2D g) {
      ...
      syntaxColorer.setCode(code);
      while (true) {
         String token = syntaxColorer.getNextToken(g);
         if (token.equals("")) break;
         if (token.equals("\n")) {
            y = (int) Math.round(y + dy);
            x = (int) Math.round(sf * x0);
         } else {
   ...
   protected FontMetrics getScaledMetrics(Font font) {
      BufferedImage bi = new BufferedImage(1, 1, BufferedImage.TYPE_INT_ARGB);
      Graphics2D g = bi.createGraphics();
      g.setRenderingHint(RenderingHints.KEY_TEXT_ANTIALIASING,
                         RenderingHints.VALUE_TEXT_ANTIALIAS_ON);
      g.setFont(font.deriveFont((float) (font.getSize() * sf)));

 ...
 *
 * @return The value <code>true</code> if the path is empty.
 */
 ...
 */
   ...
   public Point2D getFirstPoint() {
      ...
      return elements.get(0).adjustBounds(bb, pt);
   }
...
abstract class PathElement {
   ...
   public abstract Point2D adjustBounds(Rectangle2D bb, Point2D pt);
...
class MoveToElement extends PathElement {
   ...
   @Override
   public Point2D adjustBounds(Rectangle2D bb, Point2D pt) {
      if (bb.isEmpty()) {
         bb.setRect(x, y, 0, 0);
      } else {
         bb.add(x, y);
...
class ArcToElement extends PathElement {
   ...
   public Point2D adjustBounds(Rectangle2D bb, Point2D pt) {
      ...
      double cx = x - rx * Math.cos(theta);
      double cy = y + ry * Math.sin(theta);
      theta = Math.toRadians(start + sweep);
